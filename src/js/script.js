...
(function() {
  ...
  const start = function(mouse, event) {
    if (mouse) {
      if (
        event.target.dataset.note === event.target.dataset.note &&
        event.target.dataset.octave === event.target.dataset.octave
      ) {
  ...
  const stop = function(mouse, event) {
    if (mouse) {
      ...
      keyboard.keyUp(event.keyCode, () => {
        document
          .querySelector(".keyboard__key--active")
  ...
  document
    .querySelector('[data-controll="vco1.detune"]')
    .addEventListener("input", event => {
      synth.vco1.detune = event.target.value;
    });

  document
    .querySelector('[data-controll="vco1.octaveUp"]')
    .addEventListener("input", event => {
      synth.vco1.octaveUp = event.target.value;
    });

  document
    .querySelector('[data-controll="vco1.amp"]')
    .addEventListener("input", event => {
      synth.vco1.amp = event.target.value;
    });

  document
    .querySelector('[data-controll="vco1.wave"]')
    .addEventListener("input", event => {
      synth.vco1.shape = event.target.value;
      event.target.blur();
    });

  document
    .querySelector('[data-controll="vco2.detune"]')
    .addEventListener("input", event => {
      synth.vco2.detune = event.target.value;
    });

  document
    .querySelector('[data-controll="vco2.octaveUp"]')
    .addEventListener("input", event => {
      synth.vco2.octaveUp = event.target.value;
    });

  document
    .querySelector('[data-controll="vco2.amp"]')
    .addEventListener("input", event => {
      synth.vco2.amp = event.target.value;
    });

  document
    .querySelector('[data-controll="vco2.wave"]')
    .addEventListener("input", event => {
      synth.vco2.shape = event.target.value;
      event.target.blur();
    });

  document
    .querySelector('[data-controll="lfo.rate"]')
    .addEventListener("input", event => {
      synth.lfo.rate = event.target.value;
    });

  document
    .querySelector('[data-controll="lfo.amp"]')
    .addEventListener("input", event => {
      synth.lfo.amp = event.target.value;
    });
  document
    .querySelector('[data-controll="lfo.wave"]')
    .addEventListener("input", event => {
      synth.lfo.shape = event.target.value;
      event.target.blur();
    });

  document
    .querySelector('[data-controll="vca.gain"]')
    .addEventListener("input", event => {
      synth.gain.gain.value = parseFloat(event.target.value);
    });

  document
    .querySelector('[data-controll="filter.frequency"]')
    .addEventListener("input", event => {
      synth.filter.frequency.value = parseFloat(event.target.value);
    });
})();
